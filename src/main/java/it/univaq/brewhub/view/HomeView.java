...
public class HomeView {
    ...
    public Parent getView() {
        ...
        searchField.textProperty().addListener((obs, oldVal, newVal) -> {
            ...
            // Ricerca asincrona con debouncing implicito
            AsyncTaskHelper.runAsync(
                    ...
                    results -> {
                        ...
                        if (results.isEmpty()) {
                            ...
                            for (Utente u : results) {
                                ...
                                // Aggiunge un badge di verifica se l'utente Ã¨ un Torrefattore
                    ...
                    error -> {
                        Log.error("Errore ricerca utenti", error);
                        searchDropdown.hide();
                    });
        });
        ...
        Runnable refreshBadge = () -> {
            AsyncTaskHelper.runAsync(
                    ...
                    error -> Log.error("Errore conteggio notifiche", error));
        };
            ...
            AsyncTaskHelper.runAsync(
                    ...
                    notifiche -> {
                        ...
                        if (notifiche.isEmpty()) {
                            ...
                            for (it.univaq.brewhub.model.Notifica n : notifiche) {
                                ...
                                btnDelete.getStyleClass().add("notification-delete-btn");
                                ...
                                btnDelete.setOnAction(ev -> {
                                    try {
                                        ...
                                        Log.error("Errore eliminazione notifica", ex);
                                    }
                                });

                                container.getChildren().addAll(contentBox, btnDelete);
                                ...
                                contentBox.setOnMouseClicked(ev -> {
                                    if (!n.isLetto()) {
                                        try {
                                            ...
                                            Log.error("Errore marcatura notifica", ex);
                                        }
                                    }
                                });
                    ...
                    error -> {
                        Log.error("Errore caricamento notifiche", error);
                        MenuItem errorItem = new MenuItem("Errore caricamento notifiche");
                        errorItem.setDisable(true);
                        notifDropdown.getItems().add(errorItem);
                        notifDropdown.show(btnNotifiche, Side.BOTTOM, 0, 0);
                    });
        });
    ...
    private void openCreatePostWindow() {
        ...
        cbxCategoria.getItems().addAll(categoriaService.getAllCategories());
        ...
        Runnable publishAction = () -> {
            ...
            if (titolo.isBlank()) {
                DialogUtils.showError("Manca qualcosa!", "Il titolo Ã¨ obbligatorio per pubblicare.", postStage);
                return;
            }
            ...
            try {
                ...
                DialogUtils.showInfo("Post Pubblicato!", "Il tuo post Ã¨ stato pubblicato con successo.", postStage);
                postStage.close();
                loadFeed(); // Assuming loadFeed() refreshes the main feed, or a more specific
                            // loadPersonalFeed() if available
            } catch (Exception e) {
                Log.error("Errore creazione post", e);
                DialogUtils.showError("Errore", "Impossibile pubblicare il post: " + e.getMessage(), postStage);
            }
        };
        ...
        try {
            ...
            if (posts.isEmpty()) {
                ...
                if (utenteLoggato.getTipo() != Utente.TipoUtente.OSPITE) {
                    ...
                    Label lblTitle = new Label("Il tuo feed Ã¨ vuoto \uD83C\uDF43");
    ...
    private void loadFeedFollowed() {
        ...
        try {
            List<Post> posts = postService.getFeedForUser(utenteLoggato.getUsername());
            if (posts.isEmpty()) {
            ...
            Log.error("Errore feed personale", e);
        }
    }
    ...
    private void loadFeedLiked() {
        ...
        try {
            List<Post> posts = postService.getLikedPosts(utenteLoggato.getUsername());
            if (posts.isEmpty()) {
    ...
    private void loadFeedByCategory(Categoria c) {
        ...
        try {
            List<Post> posts = postService.getPostsByCategory(c.getId());
            if (posts.isEmpty()) {
            ...
            Log.error("Errore feed categoria", e);
        }
    }
    ...
    private void loadFeedByTorrefattori() {
        ...
        try {
            List<Post> posts = postService.getPostsByUserType(Utente.TipoUtente.TORREFATTORE.name());
            if (posts.isEmpty()) {
                posts = postService.getPostsByUserType(Utente.TipoUtente.TORREFATTORE.toString());
            }
            ...
            Log.error("Errore feed Torrefattori", e);
        }
    }
    ...
    private void loadFeedBySaved() {
        ...
        try {
            List<Post> posts = userService.getSavedPosts(utenteLoggato.getUsername());
            if (posts.isEmpty()) {
    ...
    private void updateSavedPostsCounter() {
        if (btnSavedPosts == null)
            return;
        try {
            int count = userService.getSavedPostsCount(utenteLoggato.getUsername());
    ...
    private void performSearch(String query) {
        ...
        try {
            List<Utente> userResults = userService.searchUsers(query);
            ...
            List<Post> postResults = postService.searchPosts(query);
    ...
    private void performUserManagementSearch() {
        ...
        try {
            List<Utente> allUsers = userService.searchUsers("");
            if (allUsers.isEmpty()) {
    ...
    private void openCategoryManagement() {
        ...
        btnSaveRef.setOnAction(e -> {
            ...
            if (name.isBlank()) {
                DialogUtils.showWarning("Attenzione", "Il nome Ã¨ obbligatorio.", stage);
                return;
            }
            try {
    ...
    private void refreshSidebar() {
        ...
        try {
            java.util.List<Categoria> cats = categoriaService.getAllCategories();
            ...
            addSeparator(sidebarContent);
            Label lblActivity = new Label("ATTIVITÃ€");
        ...
        if (utenteLoggato.getTipo() != Utente.TipoUtente.OSPITE) {
            ...
            try {
                btnSavedPosts = creaNavButton(
                        "\u2B50  Salvati (" + userService.getSavedPostsCount(utenteLoggato.getUsername()) + ")",
